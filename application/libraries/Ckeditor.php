...
class CKEditor
{
	...
	private function configSettings($config = array(), $events = array())
	{
		...
		$_config['toolbar'] = array(
			array('Source', '-', 'Bold', 'Italic', 'Underline', 'Strike', '-', 'TextColor', 'BGColor'), // Menambahkan opsi warna teks dan latar belakang
			array('Link', 'Unlink'),
			array('Image', 'Table', 'SpecialChar'),
		);


		$_config['language'] = 'en'; // Set bahasa Inggris

		// Tambahkan konfigurasi untuk dekorasi tautan
		$_config['linkStyles'] = array(
			'text-decoration' => 'underline', // Dekorasi untuk tautan
			'color' => '#0000ff', // Warna untuk tautan
	...
	private function ckeditorPath()
	{
		...
		$ckeditorUrl = str_replace('http://', 'https://', $ckeditorUrl);
